...
- Numerical/integration tests: **14 passed**. Coverage includes area-weighted precipitation budgets (wet, dry, zero-proposal fallback, nonuniform areas), transform round trips, native-cell indexing, CRPS against its pairwise definition, perfect/undefined weather scores, randomized dry ties, month-boundary pairing, disjoint splits, missing hours, train-only normalization, deterministic patches, network backward propagation, Heun integration, irregular-domain overlap coverage, exact epoch-boundary resume, label-free preparation, unit/time validation, ensemble NetCDF output and evaluation.
