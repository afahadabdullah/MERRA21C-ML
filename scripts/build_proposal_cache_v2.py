...
from merraflow.dataset_v2 import ArchiveV2, box_means_v2, candidates_v2, proposal_cache_paths

_STATE = {}


def _init(root, yy, xx, size):
    _STATE.update(root=Path(root), yy=yy, xx=xx, size=size)
...
def main():
    ...
    ids = [entry['id'] for entry in archive.index['entries']]
    scores_path, meta_path = proposal_cache_paths(archive.root, patch)
    ...
    with ProcessPoolExecutor(max_workers=args.workers, initializer=_init,
                             initargs=(str(archive.root), yy, xx, patch['size'])) as pool:
    ...
    meta_path.write_text(json.dumps(
        {'size': patch['size'], 'sampling_stride': patch['sampling_stride'],
         'candidates': int(len(yy)), 'shape': list(archive.shape), 'ids': ids}, indent=2))
